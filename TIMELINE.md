...
| 3 | Backend Core (FastAPI REST API) | ⬜ Pending |
...
- [ ] `backend/predictor.py` — loads `.joblib` at startup (not per-request)
- [ ] `backend/main.py` — FastAPI app with CORS
- [ ] `uvicorn backend.main:app --reload` runs on `localhost:8000`
- [ ] `POST /api/predict` returns correct `HIGH`/`LOW` for Patient 1/2 test cases
- [ ] `/docs` page shows all endpoints (auto-generated by FastAPI)
